...
const source = await readFile(resolve(process.cwd(), "contracts/BaseSignalBoard.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BaseSignalBoard.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BaseSignalBoard.sol"]?.BaseSignalBoard;
...
console.log(`Deploying BaseSignalBoard from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_SIGNAL_BOARD_CONTRACT_ADDRESS to this contract address in Vercel.");
